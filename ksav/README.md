...
- [x] **CI, running and green** — typecheck, 4,754 editor assertions, 494 engine
...
```sh
cd app && npm test                          # 4,754 assertions across 79 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 494 tests, 33 binaries
