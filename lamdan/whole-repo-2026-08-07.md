...
| `openFile` on a bound file | ✅ **Fixed 7 Aug** | Reads the file, finds a library entry bound to that path, **discards the text it just read**, opens the IndexedDB copy, and calls `watch.markInSync` — stamping the current on-disk mtime as agreed. `watch.ts:5-9` names Dropbox, a second window, another editor and `git checkout` as why it exists. The conflict is not missed, it is erased. **Fixed:** `openBound` compares the text it just read with the stored body and asks — take the file, or keep Ksav's copy. Not ten lines and not every piece existed: the stamp cannot answer this, because `known` is per-session, so a fresh launch has nothing to compare and `checkFile` correctly says `unknown`. The comparison is therefore on content, and keeping Ksav's copy needed a **third** state — `markConflicted`, since `forget` leaves no stamp and no stamp is deliberately read as safe. |
...
| `emit-engine.mjs` parses `lib.rs` by byte range | ✅ **Fixed 9 Aug** | `src.indexOf("impl Default for DocConfig")` then `.slice`. `services.rs:126-134` got a `#[rustfmt::skip]` and a paragraph warning that its formatting is a build input. `lib.rs:244` got nothing. Reflow it and the client silently ships different defaults. Derive `Serialize` and emit a value. **Done, and for all four tables rather than one** — the notices, the command registry and the service registry were read the same way, and `services.rs`'s `#[rustfmt::skip]` was a fence around the symptom. `engine/src/facts.rs` serialises them into `engine/facts.gen.json`; `cargo test --test facts` keeps it honest. Checked rather than claimed: `cargo fmt` was run over `services.rs`, which duly exploded all thirteen rows to seven lines each, and the generated TypeScript came out byte-identical. `runner.test.mjs` now prohibits the whole class — nothing outside `tools/facts.mjs` opens a `.rs` file. |
| `GET /inbox` destroys state | ✅ **Fixed 7 Aug** | `drain()` empties memory and truncates the file, reachable as an untokened `GET` on `ksav serve`. Any page in the writer's browser can `fetch` it; the quotation is gone from disk even though the reply can't be read. `ksav serve [addr]` takes any bind address, so on `0.0.0.0` the reply is readable too. The comment at `server.rs:432` — "it binds to loopback and holds no secrets" — was true before `/inbox` existed. |
...
| 7 | ✅ **Finished 9 Aug.** `insert.ts` landed on the 7th with 27 assertions. `header.ts` and `panelrows.ts` are the rest: twenty chip decisions, three menu decisions and four panels' row models, +103 assertions, and the chip handler table is keyed by a union so a chip with nothing to run is a `tsc` error. Four findings came out of the move — two spellings of the indent rule, a history panel listing `#שער[…]` as a version name, a palette with no empty state, and a palette silently showing 60 of 115 commands. **And one that only pressing it could find: the theme toggle never changed its glyph** — its `setSetting` branch was the one of five that did not rebuild the chrome. The file is 6,036 lines, which is not the result and was never going to be. | 7 | 2 | `rewrite` | 1 week |
...
| 10 | ✅ **Deleted 7 Aug.** What answers `/` without `embed-ui` is now a 404 naming the two commands that build the real app — strictly more useful than a stripped editor that silently is not the product, and with nothing in it to execute, so it needs no CSP exemption where the page it replaces had one. | 2 | 1 | `delete` | ~250 lines out |
...
| 12 | ✅ **Fixed 7 Aug.** `sefer` gets tiered `#מדור_א`/`#מדור_ב`, `#ציון_מקור` citations and both indexes; `divrei-torah` gets `#עם_הערות_צד` and marginal notes; `gemara` (fixed bands) and `peirush` (parallel streams) are new. `tests/templates.rs` probes all four — where the commentary landed, that the bands are on the paper, that the streams are side by side — and never `ok()`s one. | 3 | 1 | `wrong-but-keep` | 2 days |
| 13 | `_en_params` collisions: `justify`→alignment, `title` unreachable, 12 params English-less. | 10 | 2 | `rewrite` | ~1 day |
| 14 | ✅ **Fixed 7 Aug.** The blocked cells are a table, so a cell that is neither offered nor refused fails the suite; both false refusals are cards. `spec.md` opens with the ground rule instead of a count, and the count is **thirteen cards, measured from `NOTE_CHOICES`** — the fence stops choosing which artifact to count. | 4 | 1 | `wrong-but-keep` | ~40 lines |
...
| 17 | ✅ **Fixed 7 Aug.** And the sweep found the same wrong key a second time, in `spec.md`, which is the argument for building it: every `Ctrl+…` in a living page must be on the generated card **and** the paragraph around it must say what the card says that key does. `ksav/DESIGN.md` takes the sections whose subject is how this repository stopped being wrong. | 14 | 3 | `rewrite` | 6 chars + 25 lines |
...
| 20 | ✅ **Fixed 9 Aug.** The prelude is a `Source` behind a `OnceLock`, registered with the engine and imported by name; a compile is handed two lines and the body. Measured: the per-keystroke parse goes **3.70 ms → 0.03 ms** on one section and 4.35 ms → 0.43 ms on forty (`cargo run --release --example bench-prelude`). `enclosing_let` asks the syntax tree instead of scanning backwards for a column-0 `#let`. Two of the finding's prescriptions were not followed and the entry says why: `body_offset` shrank to a two-line header rather than dying, because Typst gives an `#include`d file its own scope; and the catalogue went into the prelude module rather than `sys.inputs`, which costs a `Library` clone per compile. The export still inlines the prelude and is now compiled through a resolver-less engine to prove it stands alone. | 9 | 2 | `wrong-but-keep` | 1 week |
| 21 | ✅ **Fixed 7 Aug.** `logical_text`, `page_text` and `COMMAND_CATEGORY` are gone. `bench-structure.mjs` is **not**: the report itself said a bare delete does not survive the standing rule, and its want — measuring the keystroke path at sefer scale — was real and unserved, so it was rebuilt and wired to `npm run bench`. `readme.test.mjs` has live assertions and stays. | 5 | 1 | `delete` | ~250 lines out |
