...
- [x] **CI, running and green** — typecheck, 4,009 editor assertions, 446 engine
...
```sh
cd app && npm test                          # 4,009 assertions across 66 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 446 tests, 30 binaries
