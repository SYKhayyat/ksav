...
| `giant_spill` | **A note taller than its whole region is cut across pages, word by word.** Fifty words into a 1.2cm region: page 1 carries `מילה01`–`מילה28` under the note number, page 2 carries `מילה29`–`מילה50` with no number, and every word is in the text layer exactly once. Before the cut the same document put all fifty on page 1 — two of the four lines clipped away — and all fifty again on page 2, so `Ctrl-F`, copy, and the DOCX export each found the note twice. |
| `giant_spill_marked` | **A bolded word does not cost a note its cut.** One asterisk used to: `_ct_text` refused anything that was not bare words, so the note went to the window, and the window *slices a line* — measured, a line split across two pages with its top half on one and its bottom half on the next. The words now come out of the markup, one atom per word carrying its own look. Every word once, the bolded word still bold and the highlighted word still highlighted, on one page. |
| `giant_spill_uncuttable` | **A note with a `box` in it keeps the window, and the window still works.** A box has no words in it and therefore no seam, which is the one case left where slicing is the honest answer: the note is emitted whole into both pages and slid up by exactly one slot, 34.02pt. The row is here to keep the *cost* visible — the whole note is in the text layer of both pages — because that is the trade the two mechanisms exist to make, not a bug waiting to be found again. |
