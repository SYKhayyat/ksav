...
| 7 | 🟡 **Started 7 Aug, not finished.** `actions.ts` took which command each shell action inserts; `insert.ts` now takes what an insertion *becomes* — refuse with a reason, a note layout with its scaffolding, or text and a caret — as a pure function with 27 assertions on it, where the insertion grid checked 1,026 strings and nothing checked the function choosing between them. `main.ts` is still 5,900 lines and the report's week is a real week: `header.ts` and the panel renderers are untouched. | 7 | 2 | `rewrite` | 1 week |
...
| 20 | ⬜ **Not started.** The one item of the twenty-one untouched on 7 Aug, and the estimate is honest: it is a week, and it changes how every compile in the product resolves its source. | 9 | 2 | `wrong-but-keep` | 1 week |
