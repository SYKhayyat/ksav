...
| `giant_spill_uncuttable` | **The same note with one bolded word keeps the window, and the window still works.** `_ct_text` answers `none` for anything that is not plain words, so there is no seam to cut at: the note is emitted whole into both pages and slid up by exactly one slot, 34.02pt. The row is here to keep the *cost* visible — the whole note is in the text layer of both pages — because that is the trade the two mechanisms exist to make, not a bug waiting to be found again. |
