...
| [2026-08-17 · A clamp is not a mapping](2026-08-17-a-clamp-is-not-a-mapping.md) | A long kuntres written in the assembled app: a table of contents that printed above the document's own title, and eighteen table operations that returned the old caret offset clamped into the new text — always legal, almost never right, and one keystroke from destroying the table. The same class swept next door found six more in the lists and three in the headings. Plus a dialog that refused, wiped its own refusal and closed; the deploy workflow that fires only on tags, to an environment that allows only branches; and a notes drawer numbering notes in a series that is on no page |
