...
- [x] **CI, running and green** — typecheck, 4,694 editor assertions, 491 engine
...
```sh
cd app && npm test                          # 4,694 assertions across 79 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 491 tests, 32 binaries
