...
- [x] **CI, running and green** — typecheck, 4,680 editor assertions, 479 engine
...
```sh
cd app && npm test                          # 4,680 assertions across 79 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 479 tests, 31 binaries
