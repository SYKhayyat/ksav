...
- [x] **CI, running and green** — typecheck, 4,637 editor assertions, 470 engine
...
```sh
cd app && npm test                          # 4,637 assertions across 78 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 470 tests, 31 binaries
