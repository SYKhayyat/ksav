...
fn english_gets_english_quotation_marks() {
    let text = page_text(&render("He said \"hello\" and 'goodbye'.", &cfg("ltr")));
    assert!(
...
fn hebrew_keeps_hebrew_quotation_marks() {
    ...
    let text = page_text(&render("אמר \"שלום\" ויצא.", &cfg("rtl")));
    assert!(
...
fn justified_english_is_hyphenated() {
    ...
    let text = page_text(&render(body, &cfg("ltr")));
    assert!(
...
fn contents_heading_follows_the_document_language() {
    let english = page_text(&render("= A Chapter\n\n#תוכן()\n", &cfg("ltr")));
    assert!(
    ...
    let hebrew = page_text(&render("= פרק א\n\n#תוכן()\n", &cfg("rtl")));
    assert!(
...
fn an_explicit_language_overrides_the_direction() {
    ...
    let text = page_text(&render("Er sagte \"hallo\".", &c));
