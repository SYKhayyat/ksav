...
- [x] **CI, running and green** — typecheck, 4,098 editor assertions, 451 engine
...
```sh
cd app && npm test                          # 4,098 assertions across 68 files
cd app && npm test -- panels spans          # just those files, by substring
cd app && npx tsc --noEmit                  # typecheck
cargo test --manifest-path engine/Cargo.toml            # 451 tests, 30 binaries
